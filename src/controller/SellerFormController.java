...
public class SellerFormController implements Initializable {
    ...
    @FXML
    private TextField listingIdTxt; // TextField variable for the listingNumberTxt

    @FXML
    private TextField searchTxt; // TextField variable for the listingNumberTxt
    ...
    @FXML
    private TableColumn<Customers, String> listingIdCol; // TableColumn to hold the propertyTypeCol
    ...
    void onActionAdd(ActionEvent event) {
        ...
        try { // Possible exceptions that may occur
            if (!(customerIdTxt.getText().isEmpty())){
                ...
                Customers.createCustomers(new Sellers(id, name, address, postalCode, phone, null, createdBy,
                        null, lastUpdateBy, divisionID, division, countryID, country, type, listingID));
                ...
                Customers.clearCustomers();// Resets the customersTableView with the new list
    ...
    void onActionClear(ActionEvent event) {
        ...
        listingIdTxt.clear(); // Clears listingID
        searchTxt.clear(); // Clears search
        customersTableView.getSelectionModel().clearSelection(); // Clears customersTableView
        Customers.clearCustomers(); // Clears customersList for reset
        customersTableView.setItems(Customers.getSellers()); // repopulates tableView
    ...
    void onActionDelete(ActionEvent event) {
        ...
        if (customersTableView.getSelectionModel().isEmpty()) { // Checks to see if a selection has been made
            ...
            if (result.isPresent() && result.get() == ButtonType.CANCEL) // Returns to form if deletion is cancelled
                ...
                onActionClear(event);
                Customers.clearCustomers();// Resets the customersTableView with the new list
                Customers.getSellers(); // Resets the tableView with new list
            }
            }
        }
    }
    ...
    /** This method is used to send the user to the buyers form
     *
     * @param event is a button
     * @throws IOException a possible exception that may occur
     */
    ...
    void onActionUpdate(ActionEvent event) {
        ...
        try {
            ...
            String type = "seller";
            String listingID = listingIdTxt.getText();
                ...
                if (customerNameTxt.getText().isEmpty()) { // Checks to see if the text box is empty
                    ...
                    Customers.updateCustomers(new Sellers(id, name, address, postalCode, phone, createDate, createdBy,
                            lastUpdate, lastUpdateBy, divisionID, division, countryID, country, type, listingID));
                    ...
                    Customers.clearCustomers();// Resets the customersTableView with the new list
                    Customers.getSellers(); // Resets the customersTableView after the event
    ...
    void onActionSearch (ActionEvent event){
        ...
        if (name.isEmpty()){
            ...
            alert.showAndWait();
            customersTableView.setItems(Customers.getSellers());
        }
        else {
            try {
                if (Pattern.matches("[a-zA-Z]+", name)) { // Determines if Text is a String
                    customersTableView.setItems(Customers.getSellerByName(name)); // Searches by customer name
                    if (Customers.getSearchRecords().isEmpty()) {  // Error dialog if customer not found
                        Alert alert = new Alert(Alert.AlertType.ERROR);
                        alert.setTitle("Error Dialog");
                        alert.setContentText("No Seller was found with that ID or Name!");
                    ...
                    customersTableView.setItems(Customers.getSellerByID(id));
        ...
        customersTableView.setRowFactory(tv -> {
            ...
            row.setOnMouseClicked(event -> {
                if(event.getClickCount() == 2 && (!row.isEmpty())) {
                    ...
                    // Sends the listingID to the listingIdTxt
                    listingIdTxt.setText(customers.getListingID());
    ...
    /**
     * This method creates a random number for the listingID and verifies that doesn't exist already.
     * @return a random number
     */
    public static String getListingID() {
        int number; // Holds the number
        int min = 1; // Sets the minimum to 1
        int max = 1000000; // Sets the maximum to 1000000
        ...
        // Verifies that the random number is not already assigned
        String s = String.valueOf(number); // Creates a string value of integer
        if (!Customers.checkListingID(s)){ // Checks to see if it exists already
            return s; // If not, returns number
        }
        else{
            return getListingID(); // If it does, calls on method again
        }
    }
    ...
    public void initialize(URL url, ResourceBundle rb) {
        ...
        // Assigns listingId to the listingIdCol
        listingIdCol.setCellValueFactory(new PropertyValueFactory<>("listingID"));
